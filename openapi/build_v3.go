...
func BuildV3(spec *openapi.APISpec, handlerMap map[string]*tree.Handler) {
	...
	for handler, handlerSpec := range handlers {
		...
		if handler.Node.Security != nil {
			secSpec := handler.Node.Security.(*openapi.SecuritySchemeObject)
			pathMethod.Security = append(
				pathMethod.Security,
				secSpec,
			)
			spec.Components.SecuritySchemes[secSpec.Type[0]] = &openapi.SecuritySchemeObject{Type: []string{"basic"}}
		}
